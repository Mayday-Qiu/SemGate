...
class AgenticRouter:
    ...
    def select(
        ...
        route_hints: Optional[Sequence[Dict[str, Union[float, str]]]] = None,
    ) -> AgenticRouteDecision:
        ...
        memory_boosts = _memory_boosts(route_hints or [])
        ...
        for profile in workflow_profiles:
            ...
            if score.executable and profile.workflow_id in memory_boosts:
                boost = memory_boosts[profile.workflow_id]
                score.memory_route_boost = boost
                score.match_score = round(min(1.0, score.match_score + boost), 6)
                score.score = round(score.score + boost, 6)
                score.selection_reason = f"{score.selection_reason}; memory_route_boost={boost:.2f}"
        ...
        if selected_workflow is None:
            ...
            selected_score = next((score for score in candidate_scores if score.workflow_id == selected_workflow), None)
            if selected_score is not None and selected_score.memory_route_boost > 0:
                selection_reason = f"{selection_reason}; memory_route_boost={selected_score.memory_route_boost:.2f}"
        ...
        return AgenticRouteDecision(
            ...
            score_breakdown={
                ...
                "planner_memory": {
                    ...
                    "max_total_boost_per_workflow": 0.20,
                    "hard_filters_win": True,
                },
                "resource_balance": {
...
class WorkflowMatcher:
    def score_profile(
        ...
        resource_score = ResourceBalancer.resource_score(task_profile, workflow_profile, cost_score=cost_score)
...
class ResourceBalancer:
    ...
    def resource_score(
        task_profile: TaskProfile,
        workflow_profile: WorkflowProfile,
        runtime_snapshot: Optional[RuntimeMetricsSnapshot] = None,
        cost_score: Optional[float] = None,
    ) -> float:
        effective_avg_latency_ms = workflow_profile.avg_latency_ms
        effective_error_rate = workflow_profile.error_rate
        ...
        if cost_value is None:
            cost_value = ResourceBalancer.cost_score(task_profile.cost_budget, workflow_profile.cost_level)
        score = (
            0.45 * latency_score
    ...
    def resource_score_with_runtime(
        ...
        return ResourceBalancer.resource_score(
            task_profile,
            workflow_profile,
            runtime_snapshot=runtime_snapshot,
            cost_score=candidate_score.cost_score,
        )
...
def _memory_boosts(route_hints: Sequence[Dict[str, Union[float, str]]]) -> Dict[str, float]:
    boosts: Dict[str, float] = {}
    for hint in route_hints:
        workflow_id = str(hint.get("workflow_id", ""))
        if not workflow_id:
            continue
        try:
            boost = float(hint.get("score_boost", 0.0))
        except (TypeError, ValueError):
            continue
        boosts[workflow_id] = round(min(MAX_MEMORY_ROUTE_BOOST, boosts.get(workflow_id, 0.0) + max(0.0, boost)), 6)
    return boosts
