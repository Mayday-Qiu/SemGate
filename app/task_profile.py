...
class TaskProfileBuilder:
    ...
    def _infer_task_type(self, request: AgenticGatewayRequest) -> tuple[AgenticTaskType, List[str]]:
        ...
        if self._contains_any(text, incident_keywords):
            reasons.append("matched incident/service-status keywords")
            return "incident_analysis", reasons
        if self._contains_any(text, doc_review_keywords):
            reasons.append("matched document-review keywords")
            return "doc_review", reasons
        ...
        if request.task_type in {"knowledge_qa", "doc_review", "incident_analysis", "media_generation"}:
            reasons.append("used client task_type hint because no stronger rule matched")
            return request.task_type, reasons
        if request.task_type == "tool_task":
            reasons.append("used client tool_task hint")
            return "tool_task", reasons

        reasons.append("defaulted to chat")
        return "chat", reasons
    ...
    def _profile_seed(self, task_type: AgenticTaskType, request: AgenticGatewayRequest) -> Dict[str, Any]:
        seeds: Dict[str, Dict[str, Any]] = {
            ...
            "doc_review": {
                "required_capabilities": ["inspection", "rag", "risk_check", "citation", "evidence_check"],
                "required_tools": ["inspection_tool", "doc_search_tool", "evidence_check_tool"],
                "evidence_required": True,
                "risk_level": "high",
                "permission_scope": [
                    "kb:project_docs:read",
            ...
            "incident_analysis": {
                "required_capabilities": ["business_query", "rag", "summary", "fallback", "evidence_check"],
                "required_tools": [
                    "business_query_tool",
                    "service_status_tool",
                    "doc_search_tool",
