...
from services.tool_service.registry import registry
...
app = FastAPI(title="SemRoute Tool Service", version="0.7.0")
...
async def tools() -> Dict[str, Any]:
    return {"tools": [definition.model_dump(mode="json") for definition in registry.all()]}
...
async def invoke(request: ToolInvokeRequest) -> ToolInvokeResponse:
    ...
    argument_model = registry.argument_model(request.tool_name)
